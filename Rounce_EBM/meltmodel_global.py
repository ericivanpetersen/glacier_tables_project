...
from Rounce_EBM.spc_split_lists import split_list
...
def calc_surface_fluxes(Td_i, Tair_i, RH_AWS_i, u_AWS_i, Sin_i, Lin_AWS_i, Rain_AWS_i, snow_i, P, Albedo, k,
                        ...
                        debris_thickness,
                        option_snow=0, option_snow_fromAWS=0, i_step=None, EF=1, EF_SW=1):
    ...
    # Second option: Snow depth is prescribed from AWS, so don't need to melt snow
        ...
        if Rain_AWS_i > 0:
            ...
            LE_i = 0
        Rn_i = Sin_i * (1 - Albedo) * EF_SW + debris_prms.emissivity * (Lin_AWS_i - (5.67e-8 * Td_i[0]**4)) * EF
        ...
        if Rain_AWS_i > 0:
            ...
            dLE_i = 0
        dRn_i = -4 * debris_prms.emissivity * 5.67e-8 * Td_i[0]**3 * EF
        dH_i = -1 * debris_prms.density_air_0 * P / debris_prms.P0 * debris_prms.cA * a_neutral_debris * u_AWS_i * EF
